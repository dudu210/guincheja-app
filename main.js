...
function providerTrip() {
  ...
  const goingToClient = step < 3;
  const navigationDestination = goingToClient && state.providerJob.cliente_latitude ? `${state.providerJob.cliente_latitude},${state.providerJob.cliente_longitude}` : state.providerJob.destino;
  ...
  return `${header("Corrida atual", true)}<main class="content"><div id="provider-live-map" class="real-map tracking-real-map" role="application" aria-label="Mapa da corrida atual"></div><section class="card"><div class="eyebrow" style="color:var(--navy2)">NAVEGAÇÃO • ${goingToClient ? "ATÉ O CLIENTE" : "ATÉ O DESTINO"}</div><div class="grid"><a class="btn primary navigation-button" href="${googleMapsUrl}" target="_blank" rel="noopener">ABRIR NO GOOGLE MAPS</a><a class="btn secondary navigation-button" href="${wazeUrl}" target="_blank" rel="noopener">ABRIR NO WAZE</a></div><p class="legal">Escolha o GPS com o veículo parado antes de iniciar a rota.</p></section><section class="security-note"><b>📡 GPS sendo compartilhado</b><span>Sua posição é enviada somente ao cliente deste chamado.</span></section><section class="card"><div class="timeline">${labels.map((x, i) => `<div class="step ${i < step ? "done" : i === step ? "active" : ""}">${x}</div>`).join("")}</div></section><button class="btn primary" id="provider-next">${action}</button></main>${providerNav("jobs")}`;
}
